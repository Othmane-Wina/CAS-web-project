...
router.put('/members/move-to-admin', adminOnlyMiddleware, async (req, res) => {
    ...
    try {
        ...
        if (user.role === 'ADMIN') return res.status(403).send({ message: "Cannot promote an admin." });
...
router.get('/admins', authMiddleware, async (req, res) => {
    try {
        ...
        const formattedAdmins = admins.map(admin => ({
            ...
            grade: admin.members[0]?.grade || null,
            stream: admin.members[0]?.stream || null,
            role: admin.role
        }));
